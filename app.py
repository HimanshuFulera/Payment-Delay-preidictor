...
xgb_model = pickle.load(open("rf_model.pkl", "rb"))  # Renamed for clarity
...
# Define Jinja filter for currency formatting
...
def index():
    if request.method == "POST":
        ...
        # Create input DataFrame with feature names to avoid scaler warning
        feature_names = [
            "Vendor ID", "Payment Amount", "Vendor Category", "Vendor Location",
            "Historical Payment Behavior", "Payment Terms",
            "Terms_Amount_Interaction", "Amount_Category"
        ]
        input_data = pd.DataFrame(
            [[vendor_id_enc, payment_amount, category_enc, location_enc,
        ...
        predicted_delay = xgb_model.predict(input_scaled)[0]
